...
function cipher(){
  for(i=0;i<=count;i++) {
    ...
    if(num_letra>=65 && num_letra<=90)
      {num = 65;
       cipher_word = cipher_word + String.fromCharCode((array[i].charCodeAt(0) - num + 7) % 26 + num);
      }
...
function decipher(){
  ...
  for(i=0;i<=count;i++) {
    ...
    if(num_letra>=65 && num_letra<=90)
      {num = 65;
        var condition = (array[i].charCodeAt(0)-num) - 7;
        if(condition < 0){
            decipher_word = decipher_word + String.fromCharCode((array[i].charCodeAt(0) +26 - 7));
        }
        else{
            decipher_word = decipher_word + String.fromCharCode((array[i].charCodeAt(0) - num - 7 ) % 26 + num);
        }
      }
    if(num_letra>=97 && num_letra<=122)
      {num = 97;
